        ...
        <div style="float: left; width:46%; margin: 0;">
            <div style="font-family:Arial, Helvetica, sans-serif;">
                ...
                <div style="font-size: 10px; width: 50%; float: right;">
                    <p style="margin-top: 2px; margin-left: 10px;">{{date('F', mktime(0, 0, 0, $month, 10)) }}-{{$year}}</p>
            ...
            <table style="font-size: 9px; margin-top: 45px; padding: 10px; font-family:Arial, Helvetica, sans-serif">
                ...
                @endforeach
                    <!-- @for ($i=1; $i <= 31; $i++)
                        <tr class="text-center">
                            <td>{{ $i }}</td>
                            @php $i = str_pad($i, 2, '0', STR_PAD_LEFT); @endphp
                            @if($i <= 15)
                                @if(isset($lists[$search.$i]))
                                    @if($lists[$search.$i]['day'] == 'Saturday' || $lists[$search.$i]['day'] == 'Sunday')
                                    @else
                                    <td>{{ isset($lists[$search.$i]) ? $lists[$search.$i]['am_in'] : '' }}</td>
                                    <td>{{ isset($lists[$search.$i]) ? $lists[$search.$i]['am_out'] : '' }}</td>
                                    <td>{{ isset($lists[$search.$i]) ? $lists[$search.$i]['pm_in'] : '' }}</td>
                                    <td>{{ isset($lists[$search.$i]) ? $lists[$search.$i]['pm_out'] : '' }}</td>
                                    <td></td>
                                    <td></td>
                                    @endif
                                @else
                                    @if(date('l',strtotime($search.$i)) == 'Saturday' || date('l',strtotime($search.$i)) == 'Sunday')
                                        <td colspan="6" style="letter-spacing: 3px; background: rgba(128,128,128, .2)">NON-WORKING DAY</td>
                                    @elseif($year.'-'.$month.'-'.$i < $today)
                                        <td colspan="6" style="letter-spacing: 3px; background: rgba(128,128,128, .2)">ABSENT</td>
                                    @else
                                        <td></td>
                                        <td></td>
                                        <td></td>
                                ...
                                @endif
                            @else 
                                <td style="background: rgba(128,128,128, .2)"></td>
                                <td style="background: rgba(128,128,128, .2)"></td>
                                <td style="background: rgba(128,128,128, .2)"></td>
                                <td style="background: rgba(128,128,128, .2)"></td>
                                <td style="background: rgba(128,128,128, .2)"></td>
